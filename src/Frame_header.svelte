<script>
	...
	let ehs_mode = 'admin'; // basic / multi / admin
	let cm_mode = 'basic'; // basic / multi / admin
	...
	$: {
		switch(ehs_mode) {
			case 'basic': app_data.ehs.modules_to_paint = app_data.ehs.modules.slice(0,5); break;
			case 'multi': app_data.ehs.modules_to_paint = app_data.ehs.modules.slice(0,10); break;
			case 'admin': app_data.ehs.modules_to_paint = app_data.ehs.modules.slice(0,app_data.ehs.modules.length); break;
		}
...
<nav>
	<div class="frame">
		...
		<div class="menu-icons text-right">
			...
			<span on:click='{ () => {selected_menu_item =  selected_menu_item == 'menu_settings' ? false : 'menu_settings'}}' class="menu-icon" style="opacity:0.2"><i class="i-administration i-24"></i></span>
			<span on:click='{ () => {selected_menu_item =  selected_menu_item == 'notification' ? false : 'notification'}}' class="menu-icon"><i class="i-notification i-24"></i></span>
			<span on:click='{ () => {selected_menu_item =  selected_menu_item == 'user' ? false : 'user'}}' class="menu-icon"><i class="i-user-avatar i-24"></i></span>
			<span on:click='{ () => {selected_menu_item =  selected_menu_item == 'nav' ? false : 'nav'}}' class="menu-icon"><i class="i-menu i-24"></i></span>
		</div>
	</div>
</nav>
...
{#if menu_view_bool}
<div class="main-menu"  >
	<div class="mask" on:click='{ () => {selected_menu_item =  false}}'></div>
	
	<div class="frame">
		<div class="dd" class:in="{menu_view_in}">



			{#if menu_view_section == 'menu_settings'}
				<h4>Menu Settings for testing</h4>
				<div>
					<label><input type='checkbox' bind:group={apps} value="home"> Home (aka platform)</label>
				</div>
				<div>
					<label><input type='checkbox' bind:group={apps} value="ehs"> EcoOnline EHS</label>

					{#if apps.includes('ehs') }
						<select bind:value="{ehs_mode}">
				...
				<div>
					<label><input type='checkbox' bind:group={apps} value="cm"> Chemical Manager</label>
					{#if apps.includes('cm') }
						<select bind:value="{cm_mode}">
							<option value='basic'>Basic</option>
							<option value='multi'>Multiple modules</option>
							<option value='admin'>Admin level</option>
						</select>
				...
				<div>
					<label><input type='checkbox' bind:group={apps} value="munio"> Munio Learning</label>
				</div>
				<div>
					<label><input type='checkbox' bind:group={apps} value="crisis"> Crisis Manager</label>
				</div>
				<div>
					<label><input type='checkbox' bind:group={apps} value="staysafe"> StaySafe</label>
				</div>
				<div>
					<hr>
					View mode:<br>
					...
					<label><input type='checkbox' bind:value="{toggle_view}"> Allow user to toggle view mode </label>
				</div>			
				...
				<div class='nav-row'>
					...
					<div class='nav-content'>
						{#each apps as a}
							...
							<h2><div class="icon" style={"background-image:url('./images/svgs_clean/documents.svg')"}></div> {app_data[a].name}</h2>
							<!-- svelte-ignore a11y-missing-attribute -->
							<a class='hub-link'>Go to App</a>
							<div class='nav-item-holder'>
								{#each app_data[a].modules_to_paint as m, i}
									<div class='nav-item' style="animation-delay:{i*0.02}s" class:selected="{false}" on:click|preventDefault="{ () => {nav(m.icon);window.location.hash='#' + m.url }}">
										<div class="icon" style={"background-image:url('./images/svgs_clean/" + m.icon + ".svg')"}></div>
										<b style:color = "blue">{m.name}</b>
									</div>
								{/each}
							</div>
						{/each}
...
<style>
	...
	.nav-item {
		...
		animation: navi 0.5s ease-out forwards;
